...
int incremento(void *p){
    ...
    while(pos!=-1){
        mtx_lock(&args->mutex[args->arr->size]);

        if(*args->iterations>0){
            *args->iterations = *args->iterations-1;
            mtx_unlock(&args->mutex[args->arr->size]);
...
// Selecciona dos posiciones, a la primera le incrementa y a la segunda le decrementa
int intercambio(void *p){
    int pos1 = 0, pos2, val;
    struct args *args = p;

    while(pos1 != -1){
        mtx_lock(&args->mutex[args->arr->size + 1]);

        if(*args->iterations > 0){
            *args->iterations = *args->iterations - 1;
            mtx_unlock(&args->mutex[args->arr->size + 1]);
...
struct thrd_return createThread(int id, struct options opt, struct array *arr, mtx_t *mutex, int (funThr)(void *p), int *iteraciones) {
...
int main (int argc, char **argv) {
    ...
    int iteraciones1 = opt.iterations, iteraciones2 = opt.iterations;
    mtx_t mutex[opt.size + 2];
    ...
    for(int i = 0; i < opt.num_threads; i++) {
        thread[i] = createThread(i, opt, &arr, mutex, incremento, &iteraciones1);
        thread[i + opt.num_threads] = createThread(i + opt.num_threads, opt, &arr, mutex, intercambio, &iteraciones2);
    }
